...
`;function pt(e){return e&&Je("ha-icon")?W`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:F}function ut(e){const{config:t}=e;if(!t.header.visible)return F;const s=function(e){const t=e.config.header.icon;if(!1===t)return null;if(t)return t;const s=Qe(e.hass,e.subject);return s?.icon?s.icon:"preset_mode"===e.subject.kind?"mdi:state-machine":"mdi:tune-variant"}(e),i=!1===t.header.subtitle?null:t.header.subtitle??function(e){const{hass:t,subject:s}=e,i=Qe(t,s),n=i?.name??lt(t,"no_mode");if("preset_mode"===s.kind){const{presetMode:e}=s;if(e.source_entity){const s=t.states[e.source_entity];return`${n} · ${lt(t,"follows",{entity:s?.attributes.friendly_name??e.source_entity})}`}return n}return s.presetMode?n:`${n} · ${lt(t,"no_preset_mode")}`}(e),n=t.header.icon_color??function(e){const t=Qe(e.hass,e.subject);return t?e.config.modes.colors[t.key]:void 0}(e),{tappable:r}=e,o=t.header.automatic??"preset_mode"===e.subject.kind?function(e){const{hass:t,subject:s}=e,i=s.presetMode;if(!i?.entities.automatic)return F;const n=i.entities.automatic,r=et(t,i),o=lt(t,"automatic"),a=e=>e.stopPropagation();return W`
  ...
  `;if(!r.enabled)return o();if(r.confirm&&!e.editing)return o();if("all"===r.mode){const t=Ze(e.subject);return W`
    ...
    <div class="section rows">
      ${i}${r.confirm?Et(e):F}
      ${"picker"===r.mode?function(e){const t=Ze(e.subject);if(t.length<2)return F;const s=Xe(e.hass,e.subject),i=t.find(e=>e.key===s),n=lt(e.hass,e.config.editor.confirm?"mode":"editing"),r="dropdown"===e.config.editor.style?W`
          <select
            class="select-input"
            aria-label=${n}
