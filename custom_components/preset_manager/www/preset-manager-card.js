...
`;function ct(e){return e&&Ve("ha-icon")?W`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:V}function lt(e){const{visible:t}=e.config.modes;return"never"!==t&&("always"===t||"preset_mode"===e.subject.kind)}function dt(e,t,s){const i=t.entities.active_mode;i&&e.call(e.hass.callService("preset_manager","set_active_mode",{mode:s},{entity_id:i}))}function ht(e){if(!lt(e))return V;const t=Ke(e.subject);if(!t.length)return W`<div class="section note">
  ...
  `}(e,s,i);case"date":return _t(e,s,i,"date");case"time":return _t(e,s,i,"time");case"datetime":return _t(e,s,i,"datetime");default:return W`<span class="row-value muted">
      ...
      </span>`}}function bt(e,t){const s=e.config.values.parameters,i=new Map((s??[]).map(e=>[e.parameter,e]));return function(e,t){if(!t)return e.parameters;const s=new Map(e.parameters.map(e=>[e.key,e]));return t.map(e=>s.get(e)).filter(e=>void 0!==e)}(t,s?s.map(e=>e.parameter):null).map(e=>{const t=i.get(e.key);return{parameter:e,label:t?.name??e.name,icon:t?.icon}})}function gt(e,t){return!1===t.icon?V:t.icon?ct(t.icon):e.config.values.icons?ct(De(e.hass,t.parameter.entity)?.attributes.icon):V}function $t(e,t){const{text:s,muted:i}=function(e,t){const s=De(e.hass,t.entity);return s?s.state===ze?{text:rt(e.hass,"not_set"),muted:!0}:Re(s.state)?{text:rt(e.hass,"unavailable"),muted:!0}:{text:Be(e.hass,s),muted:!1}:{text:rt(e.hass,"unavailable"),muted:!0}}(e,t.parameter);return W`
    <div class="row">
      <div class="row-label">
        ${gt(e,t)}<span>${t.label}</span>
      </div>
      <div class="row-value ${i?"muted":""}">${s}</div>
    </div>
  `}function yt(e,t,s,i){const n=s?t.parameter.editors[s]:void 0,r=De(e.hass,n),o=function(e,t){return"number"===e&&void 0!==t&&"slider"===ft(t,"mode","box")}(t.parameter.type,r);return W`
    <div class="row ${o?"wide":""}">
      <div class="row-label">
        ${gt(e,t)}<span>${i}</span>
      </div>
      <div class="row-control">
        ${vt(e,t.parameter.type,r,i)}
      </div>
    </div>
  ...
  `:V}(i)}
    ...
    `}};t([ue()],Ct.prototype,"_config",void 0),t([ue()],Ct.prototype,"_structure",void 0),Ct=t([le(`${e}-editor`)],Ct),window.customCards=window.customCards??[],window.customCards.some(t=>t.type===e)||window.customCards.push({type:e,name:"Preset Manager",description:"The values of a preset, or the modes of a preset mode, with the mode it is on right now.",preview:!1,documentationURL:"https://github.com/julezdean/ha-preset-manager"}),console.info("%c PRESET-MANAGER-CARD %c 0.3.0b5 ","color: white; background: #03a9f4; font-weight: 700;","color: #03a9f4; background: white; font-weight: 700;");
