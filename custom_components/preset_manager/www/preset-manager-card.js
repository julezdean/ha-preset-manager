...
`;function lt(e){return e&&Ve("ha-icon")?W`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:V}function dt(e){const{visible:t}=e.config.modes;return"never"!==t&&("always"===t||"preset_mode"===e.subject.kind)}function ht(e,t,s){const i=t.entities.active_mode;i&&e.call(e.hass.callService("preset_manager","set_active_mode",{mode:s},{entity_id:i}))}function ut(e){if(!dt(e))return V;const t=Xe(e.subject);if(!t.length)return W`<div class="section note">
      ${ot(e.hass,"no_modes")}
    </div>`;const s=function(e){return e.subject.presetMode}(e),i=Ke(e.hass,e.subject),n=null!==function(e,t){return t?t.source_entity?"external":t.entities.active_mode?Ze(e,t)?"automatic":null:"missing":"missing"}(e.hass,s);return W`
    <div class="section">
      ${"dropdown"===e.config.modes.style?function(e,t,s,i,n){return W`
    <select
      class="select-input"
      aria-label=${ot(e.hass,"preset_mode")}
      ?disabled=${n}
      @change=${t=>{const s=t.target.value;i&&ht(e,i,s)}}
    >
      ${null===s?W`<option value="" selected>${ot(e.hass,"no_mode")}</option>`:V}
      ${t.map(e=>W`
    ...
    <div class="chips" role="group">
      ${t.map(t=>{const o=t.key===s,a=r.modes.colors[t.key];return W`
          <button
            ...
            @click=${()=>i&&ht(e,i,t.key)}
          >
            ${r.modes.icons?lt(t.icon):V}
            <span>${t.name}</span>
          </button>
        `})}
    </div>
  ...
  `}function pt(e){const{config:t}=e;if(!t.header.visible)return V;const s=function(e){const t=e.config.header.icon;if(!1===t)return null;if(t)return t;const s=Ye(e.hass,e.subject);return s?.icon?s.icon:"preset_mode"===e.subject.kind?"mdi:state-machine":"mdi:tune-variant"}(e),i=!1===t.header.subtitle?null:t.header.subtitle??function(e){const{hass:t,subject:s}=e,i=Ye(t,s),n=i?.name??ot(t,"no_mode");if("preset_mode"===s.kind){const{presetMode:e}=s;if(e.source_entity){const s=t.states[e.source_entity];return`${n} · ${ot(t,"follows",{entity:s?.attributes.friendly_name??e.source_entity})}`}return n}return s.presetMode?`${n} · ${s.presetMode.name}`:`${n} · ${ot(t,"no_preset_mode")}`}(e),n=t.header.icon_color??function(e){const t=Ye(e.hass,e.subject);return t?e.config.modes.colors[t.key]:void 0}(e),{tappable:r}=e,o="preset_mode"===e.subject.kind||dt(e)?function(e){const{hass:t,subject:s}=e,i=s.presetMode;if(!i?.entities.automatic)return V;const n=i.entities.automatic,r=Ze(t,i),o=ot(t,"automatic"),a=e=>e.stopPropagation();return W`
    <label
      class="switch"
      title=${o}
      @click=${a}
  ...
  `}function mt(e,t,s,i){const n=t.entity_id.split(".",1)[0];e.call(e.hass.callService(n,s,i,{entity_id:t.entity_id}))}function ft(e){return{disabled:(t=e.state,void 0===t||t===He),empty:Re(e.state)};var t}function _t(e,t,s){const i=e.attributes[t];return null==i?s:i}function vt(e,t,s,i){const{disabled:n,empty:r}=ft(t);let o=r?"":t.state;return"datetime"===i&&(o=r?"":function(e){const t=new Date(e);if(Number.isNaN(t.getTime()))return"";const s=e=>String(e).padStart(2,"0");return`${t.getFullYear()}-${s(t.getMonth()+1)}-${s(t.getDate())}T${s(t.getHours())}:${s(t.getMinutes())}`}(t.state)),"time"===i&&(o=o.slice(0,5)),W`
    <input
      class="date-input"
      ...
      .value=${o}
      ?disabled=${n}
      @change=${s=>{const n=s.target.value;n&&mt(e,t,"set_value","date"===i?{date:n}:"time"===i?{time:`${n}:00`}:{datetime:n.replace("T"," ")+":00"})}}
    />
  `}function bt(e,t,s,i){if(!s)return W`<span class="row-value muted">
      ${ot(e.hass,"unavailable")}
    </span>`;switch(t){case"number":return function(e,t,s){const{disabled:i,empty:n}=ft(t),r=_t(t,"min",0),o=_t(t,"max",100),a=_t(t,"step",1),c=t.attributes.unit_of_measurement??"",l=n?"":t.state,d=s=>{const i=s.target.valueAsNumber;Number.isNaN(i)||mt(e,t,"set_value",{value:i})};return"slider"===_t(t,"mode","box")?W`
      <input
        class="slider"
        type="range"
        aria-label=${s}
  ...
  `}(e,s,i);case"boolean":return function(e,t,s){const{disabled:i,empty:n}=ft(t);return W`
    <label class="switch">
      <input
        type="checkbox"
        role="switch"
        ...
        .indeterminate=${n}
        ?disabled=${i}
        @change=${s=>mt(e,t,s.target.checked?"turn_on":"turn_off",{})}
      />
    </label>
  `}(e,s,i);case"select":return function(e,t,s){const{disabled:i,empty:n}=ft(t),r=_t(t,"options",[]);return W`
    <select
      class="select-input"
      aria-label=${s}
      ?disabled=${i}
      @change=${s=>mt(e,t,"select_option",{option:s.target.value})}
    >
  ...
  `}(e,s,i);case"text":return function(e,t,s){const{disabled:i,empty:n}=ft(t),r=t.attributes.pattern;return W`
    <input
      class="text-input"
      type=${"password"===_t(t,"mode","text")?"password":"text"}
      aria-label=${s}
      minlength=${_t(t,"min",0)}
      maxlength=${_t(t,"max",255)}
      pattern=${r??V}
      .value=${n?"":t.state}
      ?disabled=${i}
      @change=${s=>mt(e,t,"set_value",{value:s.target.value})}
    />
  `}(e,s,i);case"date":return vt(e,s,i,"date");case"time":return vt(e,s,i,"time");case"datetime":return vt(e,s,i,"datetime");default:return W`<span class="row-value muted">
        ${ot(e.hass,"not_editable")}
      </span>`}}function gt(e,t){return"number"===e&&void 0!==t&&"slider"===_t(t,"mode","box")}function $t(e,t){const s=e.config.values.parameters,i=new Map((s??[]).map(e=>[e.parameter,e]));return function(e,t){if(!t)return e.parameters;const s=new Map(e.parameters.map(e=>[e.key,e]));return t.map(e=>s.get(e)).filter(e=>void 0!==e)}(t,s?s.map(e=>e.parameter):null).map(e=>{const t=i.get(e.key);return{parameter:e,label:t?.name??e.name,icon:t?.icon}})}function yt(e,t,s){if(!s)return V;const i=!1===t.icon?void 0:t.icon??(e.config.values.icons?De(e.hass,t.parameter.entity)?.attributes.icon:void 0);return W`<span class="row-icon">${lt(i)}</span>`}function wt(e,t,s){const{text:i,muted:n}=function(e,t){const s=De(e.hass,t.entity);return s?s.state===ze?{text:ot(e.hass,"not_set"),muted:!0}:Re(s.state)?{text:ot(e.hass,"unavailable"),muted:!0}:{text:Be(e.hass,s),muted:!1}:{text:ot(e.hass,"unavailable"),muted:!0}}(e,t.parameter);return W`
    <div class="row">
      <div class="row-label">
        ${yt(e,t,s)}<span>${t.label}</span>
      </div>
      <div class="row-value ${n?"muted":""}">${i}</div>
    </div>
  `}function xt(e,t,s,i,n){const r=s?t.parameter.editors[s]:void 0,o=De(e.hass,r),a=gt(t.parameter.type,o);return W`
    <div class="row ${a?"wide":""}">
      <div class="row-label">
        ${yt(e,t,n)}<span>${i}</span>
      </div>
      <div class="row-control">
        ${bt(e,t.parameter.type,o,i)}
      </div>
    </div>
  `}function At(e){if("preset"!==e.subject.kind)return V;if(!e.config.values.visible)return V;const t=e.subject.preset,s=$t(e,t);if(!s.length)return W`<div class="section note">
      ${ot(e.hass,"no_parameters")}
    </div>`;const i=null===e.subject.presetMode?W`<div class="note warning">${ot(e.hass,"orphaned")}</div>`:V,n=function(e,t){return e.config.values.icons||t.some(e=>"string"==typeof e.icon)}(e,s),{editor:r}=e.config;if(!r.enabled)return W`
      <div class="section rows">
        ${i}${s.map(t=>wt(e,t,n))}
      </div>
    `;if("all"===r.mode){const t=Xe(e.subject);return W`
      <div class="section rows">
        ${i}${s.map(s=>W`
            <div class="group-label">${s.label}</div>
            ${t.map(t=>xt(e,s,t.key,t.name,n))}
          `)}
      </div>
    `}const o="active"===r.mode?Ke(e.hass,e.subject):e.editMode;return W`
    <div class="section rows">
      ${i}${"picker"===r.mode?function(e){const t=Xe(e.subject);if(t.length<2)return V;const s=Ke(e.hass,e.subject),i=t.find(e=>e.key===s);return W`
    <div class="group-label">${ot(e.hass,"editing")}</div>
    <div
      class="chips secondary"
      role="group"
      aria-label=${ot(e.hass,"editing")}
    >
      ${t.map(t=>W`
          <button
            class="chip"
    ...
    ${i&&i.key!==e.editMode?W`<div class="note">
          ${ot(e.hass,"active_is",{mode:i.name})}
        </div>`:V}
  `}(e):V}
      ${s.map(t=>xt(e,t,o,t.label,n))}
    </div>
  `}function kt(e,t){const s=e.config.presets.editable;return t.parameters.map(i=>{const n=s&&gt(i.type,De(e.hass,i.editors[Ge(e.hass,t)??""]));return W`
      <div class="row ${n?"wide":""}">
        <div class="row-label"><span>${i.name}</span></div>
        ${s?function(e,t,s){const i=Ge(e.hass,t),n=De(e.hass,i?s.editors[i]:void 0);return W`
    <div class="row-control">
      ${bt(e,s.type,n,s.name)}
    </div>
  `}(e,t,i):function(e,t){const s=De(e.hass,t.entity);let i,n=!0;return s?s.state===ze?i=ot(e.hass,"not_set"):Re(s.state)?i=ot(e.hass,"unavailable"):(i=Be(e.hass,s),n=!1):i=ot(e.hass,"unavailable"),W`<div class="row-value ${n?"muted":""}">${i}</div>`}(e,i)}
      </div>
    `})}function Et(e,t){const{hass:s,subject:i}=e;switch(t){case"preset_mode":return"preset"===i.kind?i.presetMode?`${ot(s,"preset_mode")}: ${i.presetMode.name}`:ot(s,"orphaned"):at(s,"presets_one","presets_other",i.presets.length);case"blueprint":return"preset"===i.kind&&i.blueprint?`${ot(s,"blueprint")}: ${i.blueprint.name}`:null;case"source":{const e="preset_mode"===i.kind?i.presetMode.source_entity:null;if(!e)return null;const t=s.states[e];return`${ot(s,"source")}: ${t?.attributes.friendly_name??e}`}case"last_changed":{const e=De(s,Je(i));return e?`${ot(s,"changed")}: ${function(e,t){const s=new Date(t).getTime();if(Number.isNaN(s))return"";const i=Math.round((s-Date.now())/1e3),n=[["year",31536e3],["month",2592e3],["day",86400],["hour",3600],["minute",60]],r=new Intl.RelativeTimeFormat(e.language||"en",{numeric:"auto"});for(const[e,t]of n)if(Math.abs(i)>=t)return r.format(Math.round(i/t),e);return r.format(Math.round(i),"second")}(s,e.last_changed)}`:null}default:return null}}const St={action:"more-info"};let Mt=class extends ae{constructor(){super(...arguments),this._editModeOverride=null,this._watched=[],this._held=!1,this._lastTap=0}setConfig(e){this._config=Ee(e),this._editModeOverride=null,this._watched=[]}static getConfigElement(){return document.createElement(`${e}-editor`)}static async getStubConfig(t){const s=Ue(t)??await Oe(t),i=s.preset_modes.find(e=>e.entities.mode)?.entities.mode??s.presets.find(e=>e.entities.active_mode)?.entities.active_mode;return{type:`custom:${e}`,entity:i??""}}set hass(e){const t=this._hass;if(this._hass=e,!t)return this.requestUpdate(),void this._load();if(t.language===e.language&&t.themes===e.themes){for(const s of this._watched)if(t.states[s]!==e.states[s])return void this.requestUpdate()}else this.requestUpdate()}get hass(){return this._hass}connectedCallback(){super.connectedCallback(),this._load()}disconnectedCallback(){super.disconnectedCallback(),this._unsubscribe?.(),this._unsubscribe=void 0,this._clearTimers()}async _load(){this._hass&&!this._unsubscribe&&(this._unsubscribe=Ne(this._hass,e=>{this._structure=e}),this._structure=await Oe(this._hass))}getCardSize(){const e=this._config;if(!e)return 2;let t=e.header.visible?1:0;const s=this._subject;return"never"!==e.modes.visible&&("always"===e.modes.visible||null===s||"preset_mode"===s.kind)&&(t+=1),e.values.visible&&(t+=2),e.presets.visible&&(t+=2),e.footer.visible&&(t+=1),Math.max(1,t)}getGridOptions(){return{columns:12,min_columns:6}}get _tapAction(){return this._config?.tap_action??St}_clearTimers(){this._holdTimer&&clearTimeout(this._holdTimer),this._tapTimer&&clearTimeout(this._tapTimer),this._errorTimer&&clearTimeout(this._errorTimer),this._holdTimer=this._tapTimer=this._errorTimer=void 0}_headerDown(e){this._held=!1,We(this._config?.hold_action)&&(this._holdTimer=setTimeout(()=>{this._held=!0,this._run(this._config?.hold_action,e)},500))}_headerUp(){this._holdTimer&&clearTimeout(this._holdTimer),this._holdTimer=void 0}_headerClick(e){if(this._held)return void(this._held=!1);const t=this._config?.double_tap_action;if(!We(t))return void this._run(this._tapAction,e);const s=Date.now();if(s-this._lastTap<250)return this._tapTimer&&clearTimeout(this._tapTimer),this._tapTimer=void 0,this._lastTap=0,void this._run(t,e);this._lastTap=s,this._tapTimer=setTimeout(()=>{this._tapTimer=void 0,this._run(this._tapAction,e)},250)}_run(e,t){if(!this._hass)return;const s="preset_mode"===t.kind?t.presetMode.entities.mode:t.preset.entities.active_mode;this._call(qe(this,this._hass,e,s??this._config?.entity))}_call(e){e.then(()=>{void 0!==this._error&&(this._error=void 0)},e=>{this._error=this._messageOf(e),this._errorTimer&&clearTimeout(this._errorTimer),this._errorTimer=setTimeout(()=>{this._errorTimer=void 0,this._error=void 0},6e3)})}_messageOf(e){if("string"==typeof e)return e;if(e&&"object"==typeof e){const t=e,s=t.body;for(const e of[t.message,s?.message,t.error])if("string"==typeof e&&e)return e}return String(e)}willUpdate(e){super.willUpdate(e);const t=this._subject;this._watched=t?function(e){if("preset_mode"===e.kind){const t=st(e.presetMode);for(const s of e.presets)t.push(...tt(s));return t}const t=tt(e.preset);return e.presetMode&&t.push(...st(e.presetMode)),t}(t):[]}get _subject(){return this._config&&this._structure?it(this._structure,this._config.entity):null}_editMode(e){if(this._editModeOverride)return this._editModeOverride;const t=this._config?.editor.default_mode,s=Xe(e);return t&&s.some(e=>e.key===t)?t:Ke(this._hass,e)??s[0]?.key??null}render(){const e=this._config,t=this._hass;if(!e||!t)return V;if(!this._structure)return this._shell(this._skeleton());const s=this._subject;if(!s){const s=this._structure.preset_modes.length+this._structure.presets.length;return this._shell(this._alert(s?ot(t,"not_found",{entity:e.entity}):ot(t,"not_set_up")))}const i={hass:t,config:e,subject:s,host:this,editMode:this._editMode(s),selectEditMode:e=>{this._editModeOverride=e},call:e=>this._call(e),tappable:We(this._tapAction)||We(e.hold_action),onHeaderDown:()=>this._headerDown(s),onHeaderUp:()=>this._headerUp(),onHeaderClick:()=>this._headerClick(s)};return this._shell(W`
      ${pt(i)} ${ut(i)} ${At(i)}
      ${function(e){if("preset_mode"!==e.subject.kind)return V;if(!e.config.presets.visible)return V;const{presets:t}=e.subject;if(!t.length)return W`<div class="section note">
      ${at(e.hass,"presets_one","presets_other",0)}
        ...
        `;if(!s){const t=De(e.hass,i);return W`
            <div class="row">
              ...
              <div class="row-value ${t&&!Re(t.state)?"":"muted"}">
                ${t&&!Re(t.state)?t.state:ot(e.hass,"no_mode")}
              </div>
            </div>
          `}return W`
  ...
  `}(i)} ${function(e){if(!e.config.footer.visible)return V;const t=e.config.footer.content.map(t=>Et(e,t)).filter(e=>null!==e);return t.length?W`
  ...
  `:V}(i)}
    ...
    `}_alert(e){return Ve("ha-alert")?W`<ha-alert alert-type="warning">${e}</ha-alert>`:W`<div class="fallback-alert" role="alert">${e}</div>`}};Mt.styles=ct,t([ue()],Mt.prototype,"_config",void 0),t([ue()],Mt.prototype,"_structure",void 0),t([ue()],Mt.prototype,"_editModeOverride",void 0),t([ue()],Mt.prototype,"_error",void 0),Mt=t([le(e)],Mt);const Tt={entity:"Entity",header:"Header",modes:"Modes",values:"Values",editor:"Editing",presets:"Presets",footer:"Footer",actions:"Actions",visible:"Show",title:"Title",subtitle:"Subtitle",icon:"Icon",icon_color:"Icon colour",style:"Style",icons:"Show icons",parameters:"Parameters",parameters_note:"Parameters",enabled:"Editable",editable:"Editable",mode:"Which mode",default_mode:"Start on",content:"Content",tap_action:"Tap",hold_action:"Hold",double_tap_action:"Double tap"},Pt=["more-info","navigate","url","perform-action","none"];function Ct(e){return{selector:{select:{mode:"dropdown",options:e.map(([e,t])=>({value:e,label:t}))}}}}let jt=class extends ae{constructor(){super(...arguments),this._config={},this._label=e=>e.name&&Tt[e.name]||e.title||e.name||""}set hass(e){this._hass=e,this._structure??=Ue(e),this._load(),this.requestUpdate()}get hass(){return this._hass}setConfig(e){this._config=e}disconnectedCallback(){super.disconnectedCallback(),this._unsubscribe?.(),this._unsubscribe=void 0}async _load(){this._hass&&!this._unsubscribe&&(this._unsubscribe=Ne(this._hass,e=>{this._structure=e}),this._structure=await Oe(this._hass))}_entityPicker(){const e=[...(this._structure?.preset_modes??[]).map(e=>e.entities.mode),...(this._structure?.presets??[]).map(e=>e.entities.active_mode)].filter(e=>Boolean(e));return e.length?{include_entities:e}:{integration:"preset_manager"}}_hasParameterOverrides(){const e=this._config.values,t=e?.parameters;return Array.isArray(t)&&t.some(e=>"string"!=typeof e)}get _subject(){const e=this._config.entity;return this._structure&&"string"==typeof e&&e?it(this._structure,e):null}_schema(e){const t="preset"===e?.kind,s="preset_mode"===e?.kind,i=e?"preset_mode"===e.kind?e.presetMode.modes:e.preset.modes:[],n=[{name:"entity",required:!0,selector:{entity:this._entityPicker()}},{type:"expandable",name:"header",title:Tt.header,schema:[{name:"visible",selector:{boolean:{}}},{name:"title",selector:{text:{}}},{name:"subtitle",selector:{text:{}}},{name:"icon",selector:{icon:{}}},{name:"icon_color",selector:{ui_color:{}}}]},{type:"expandable",name:"modes",title:Tt.modes,schema:[{name:"visible",...Ct([["auto","Automatic"],["always","Always"],["never","Never"]])},{name:"style",...Ct([["chips","Chips"],["dropdown","Dropdown"]])},...i.some(e=>e.icon)?[{name:"icons",selector:{boolean:{}}}]:[]]}];if(t){const t=e.preset.parameters.map(e=>({value:e.key,label:e.name}));n.push({type:"expandable",name:"values",title:Tt.values,schema:[{name:"visible",selector:{boolean:{}}},this._hasParameterOverrides()?{name:"parameters_note",type:"constant",value:"Renamed parameters are edited in YAML."}:{name:"parameters",selector:{select:{multiple:!0,mode:"list",options:t}}},{name:"icons",selector:{boolean:{}}}]},{type:"expandable",name:"editor",title:Tt.editor,schema:[{name:"enabled",selector:{boolean:{}}},{name:"mode",...Ct([["picker","Pick a mode in the card"],["active","The active mode"],["all","Every mode"]])},{name:"default_mode",...Ct(i.map(e=>[e.key,e.name]))}]})}return s&&n.push({type:"expandable",name:"presets",title:Tt.presets,schema:[{name:"visible",selector:{boolean:{}}},{name:"values",selector:{boolean:{}}},{name:"editable",selector:{boolean:{}}}]}),n.push({type:"expandable",name:"footer",title:Tt.footer,schema:[{name:"visible",selector:{boolean:{}}},{name:"content",selector:{select:{multiple:!0,mode:"list",options:[{value:"preset_mode",label:"Preset mode"},{value:"blueprint",label:"Blueprint"},{value:"source",label:"Source entity"},{value:"last_changed",label:"Last change"}]}}}]},{type:"expandable",title:Tt.actions,schema:[{name:"tap_action",selector:{ui_action:{actions:Pt}}},{name:"hold_action",selector:{ui_action:{actions:Pt}}},{name:"double_tap_action",selector:{ui_action:{actions:Pt}}}]}),n}_formData(){try{const t=Ee({type:`custom:${e}`,...this._config,entity:this._config.entity??"sensor.placeholder"});return{...t,entity:this._config.entity??"",values:{...t.values,parameters:this._config.values?.parameters}}}catch(e){return{...this._config}}}_valueChanged(e){e.stopPropagation();Le(this,"config-changed",{config:Se({...e.detail.value})})}render(){if(!this._hass)return V;const e=this._subject;return W`
      <ha-form
        .hass=${this._hass}
        .data=${this._formData()}
    ...
    `}};t([ue()],jt.prototype,"_config",void 0),t([ue()],jt.prototype,"_structure",void 0),jt=t([le(`${e}-editor`)],jt),window.customCards=window.customCards??[],window.customCards.some(t=>t.type===e)||window.customCards.push({type:e,name:"Preset Manager",description:"The values of a preset, or the modes of a preset mode, with the mode it is on right now.",preview:!1,documentationURL:"https://github.com/julezdean/ha-preset-manager"}),console.info("%c PRESET-MANAGER-CARD %c 0.3.0b9 ","color: white; background: #03a9f4; font-weight: 700;","color: #03a9f4; background: white; font-weight: 700;");
