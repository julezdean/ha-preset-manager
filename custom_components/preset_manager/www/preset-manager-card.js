...
`;function pt(e){return e&&Je("ha-icon")?W`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:F}function ut(e){const{config:t}=e;if(!t.header.visible)return F;const s=function(e){const t=e.config.header.icon;if(!1===t)return null;if(t)return t;const s=Qe(e.hass,e.subject);return s?.icon?s.icon:"preset_mode"===e.subject.kind?"mdi:state-machine":"mdi:tune-variant"}(e),i=!1===t.header.subtitle?null:t.header.subtitle??function(e){const{hass:t,subject:s}=e,i=Qe(t,s),n=i?.name??lt(t,"no_mode");if("preset_mode"===s.kind){const{presetMode:e}=s;if(e.source_entity){const s=t.states[e.source_entity];return`${n} · ${lt(t,"follows",{entity:s?.attributes.friendly_name??e.source_entity})}`}return n}return s.presetMode?n:`${n} · ${lt(t,"no_preset_mode")}`}(e),n=t.header.icon_color??function(e){const t=Qe(e.hass,e.subject);return t?e.config.modes.colors[t.key]:void 0}(e),{tappable:r}=e,o=t.header.automatic??"preset_mode"===e.subject.kind?function(e){const{hass:t,subject:s}=e,i=s.presetMode;if(!i?.entities.automatic)return F;const n=i.entities.automatic,r=et(t,i),o=lt(t,"automatic"),a=e=>e.stopPropagation();return W`
    <label
      class="switch"
      title=${o}
      @click=${a}
  ...
  `}function mt(e,t,s){const i=t.entities.active_mode;i&&e.call(e.hass.callService("preset_manager","set_active_mode",{mode:s},{entity_id:i}))}function ft(e){if(!function(e){const t=e.config.modes.visible;return void 0===t?"preset_mode"===e.subject.kind:"manual"===t?null===tt(e.hass,e.subject.presetMode):"always"===t}(e))return F;const t=Ze(e.subject);if(!t.length)return W`<div class="section note">
      ${lt(e.hass,"no_modes")}
    </div>`;const s=function(e){return e.subject.presetMode}(e),i=Xe(e.hass,e.subject),n=null!==tt(e.hass,s);return W`
    <div class="section">
    ...
    >
      ${null===s?W`<option value="" selected>${lt(e.hass,"no_mode")}</option>`:F}
      ${t.map(e=>W`
  ...
  `}function _t(e,t,s,i,n){if(e.stage)return void e.stage(t.entity_id,{service:s,data:i,display:n});const r=t.entity_id.split(".",1)[0];e.call(e.hass.callService(r,s,i,{entity_id:t.entity_id}))}function vt(e,t){const s=e.draft?.get(t.entity_id)?.display;return void 0!==s?{disabled:!1,empty:!1,staged:s}:{disabled:(i=t.state,void 0===i||i===Re),empty:Le(t.state),staged:void 0};var i}function bt(e,t,s){const i=e.attributes[t];return null==i?s:i}function gt(e,t,s,i){const{disabled:n,empty:r,staged:o}=vt(e,t);let a=o??(r?"":t.state);return void 0===o&&("datetime"===i&&(a=r?"":function(e){const t=new Date(e);if(Number.isNaN(t.getTime()))return"";const s=e=>String(e).padStart(2,"0");return`${t.getFullYear()}-${s(t.getMonth()+1)}-${s(t.getDate())}T${s(t.getHours())}:${s(t.getMinutes())}`}(t.state)),"time"===i&&(a=a.slice(0,5))),W`
    ...
    />
  `}function $t(e,t,s,i){if(!s)return W`<span class="row-value muted">
      ${lt(e.hass,"unavailable")}
    </span>`;switch(t){case"number":return function(e,t,s){const{disabled:i,empty:n,staged:r}=vt(e,t),o=bt(t,"min",0),a=bt(t,"max",100),c=bt(t,"step",1),l=t.attributes.unit_of_measurement??"",d=r??t.state,h=n?"":d,p=s=>{const i=s.target.valueAsNumber;Number.isNaN(i)||_t(e,t,"set_value",{value:i},String(i))};return"slider"===bt(t,"mode","box")?W`
      <input
        class="slider"
        type="range"
        aria-label=${s}
  ...
  `}(e,s,i);case"boolean":return function(e,t,s){const{disabled:i,empty:n,staged:r}=vt(e,t);return W`
    <label class="switch">
      <input
        type="checkbox"
        role="switch"
  ...
  `}(e,s,i);case"select":return function(e,t,s){const{disabled:i,empty:n,staged:r}=vt(e,t),o=bt(t,"options",[]),a=r??t.state;return W`
    <select
      class="select-input"
      aria-label=${s}
      ?disabled=${i}
  ...
  `}(e,s,i);case"text":return function(e,t,s){const{disabled:i,empty:n,staged:r}=vt(e,t),o=t.attributes.pattern;return W`
    <input
      class="text-input"
    ...
    />
  `}(e,s,i);case"date":return gt(e,s,i,"date");case"time":return gt(e,s,i,"time");case"datetime":return gt(e,s,i,"datetime");default:return W`<span class="row-value muted">
        ${lt(e.hass,"not_editable")}
      </span>`}}function yt(e,t){return"number"===e&&void 0!==t&&"slider"===bt(t,"mode","box")}function wt(e,t){const s=e.config.values.parameters,i=new Map((s??[]).map(e=>[e.parameter,e]));return function(e,t){if(!t)return e.parameters;const s=new Map(e.parameters.map(e=>[e.key,e]));return t.map(e=>s.get(e)).filter(e=>void 0!==e)}(t,s?s.map(e=>e.parameter):null).map(e=>{const t=i.get(e.key);return{parameter:e,label:t?.name??e.name,icon:t?.icon}})}function xt(e,t,s){if(!s)return F;const i=!1===t.icon?void 0:t.icon??(e.config.values.icons?Be(e.hass,t.parameter.entity)?.attributes.icon:void 0);return W`<span class="row-icon">${pt(i)}</span>`}function At(e,t,s){const{text:i,muted:n}=function(e,t){const s=Be(e.hass,t.entity);return s?s.state===De?{text:lt(e.hass,"not_set"),muted:!0}:Le(s.state)?{text:lt(e.hass,"unavailable"),muted:!0}:{text:We(e.hass,s),muted:!1}:{text:lt(e.hass,"unavailable"),muted:!0}}(e,t.parameter);return W`
    <div class="row">
      <div class="row-label">
  ...
  `}function kt(e,t,s,i,n){const r=s?t.parameter.editors[s]:void 0,o=Be(e.hass,r),a=yt(t.parameter.type,o);return W`
  ...
  `}function Et(e){const t=lt(e.hass,"editing");return W`
    <div class="toolbar">
  ...
  `}function Mt(e){if("preset"!==e.subject.kind)return F;if(!e.config.values.visible)return F;const t=e.subject.preset,s=wt(e,t);if(!s.length)return W`<div class="section note">
      ${lt(e.hass,"no_parameters")}
    </div>`;const i=null===e.subject.presetMode?W`<div class="note warning">${lt(e.hass,"orphaned")}</div>`:F,n=function(e,t){return e.config.values.icons||t.some(e=>"string"==typeof e.icon)}(e,s),{editor:r}=e.config,o=()=>W`
  ...
  `;if(!r.enabled)return o();if(r.confirm&&!e.editing)return o();if("all"===r.mode){const t=Ze(e.subject);return W`
    ...
    <div class="section rows">
      ${i}${r.confirm?Et(e):F}
      ${"picker"===r.mode?function(e){const t=Ze(e.subject);if(t.length<2)return F;const s=Xe(e.hass,e.subject),i=t.find(e=>e.key===s),n=lt(e.hass,"editing"),r="dropdown"===e.config.editor.style?W`
          <select
            class="select-input"
            aria-label=${n}
  ...
  `}function Tt(e,t){const s=e.config.presets.editable;return t.parameters.map(i=>{const n=s&&yt(i.type,Be(e.hass,i.editors[Ye(e.hass,t)??""]));return W`
  ...
  `}(e,t,i):function(e,t){const s=Be(e.hass,t.entity);let i,n=!0;return s?s.state===De?i=lt(e.hass,"not_set"):Le(s.state)?i=lt(e.hass,"unavailable"):(i=We(e.hass,s),n=!1):i=lt(e.hass,"unavailable"),W`<div class="row-value ${n?"muted":""}">${i}</div>`}(e,i)}
      </div>
  ...
  `}(i)} ${function(e){if(!e.config.footer.visible)return F;const t=e.config.footer.content.map(t=>Ct(e,t)).filter(e=>null!==e);return t.length?W`
