...
`;function ut(e){return e&&Ve("ha-icon")?W`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:F}function mt(e){const{config:t}=e;if(!t.header.visible)return F;const s=function(e){const t=e.config.header.icon;if(!1===t)return null;if(t)return t;const s=Ze(e.hass,e.subject);return s?.icon?s.icon:"preset_mode"===e.subject.kind?"mdi:state-machine":"mdi:tune-variant"}(e),i=!1===t.header.subtitle?null:t.header.subtitle??function(e){const{hass:t,subject:s}=e,i=Ze(t,s),n=i?.name??dt(t,"no_mode");if("preset_mode"===s.kind){const{presetMode:e}=s;if(e.source_entity){const s=t.states[e.source_entity];return`${n} · ${dt(t,"follows",{entity:s?.attributes.friendly_name??e.source_entity})}`}return n}return s.presetMode?!1===tt(t,s)?`${n} · ${dt(t,"manual")}`:n:`${n} · ${dt(t,"no_preset_mode")}`}(e),n=t.header.icon_color??function(e){const t=Ze(e.hass,e.subject);return t?e.config.modes.colors[t.key]:void 0}(e),{tappable:r}=e,o=function(e){const{hass:t,subject:s,config:i}=e;if(!i.header.automatic)return F;const n=Qe(s);if(!n)return F;const r=tt(t,s),o=dt(t,"mode_automatic");return W`
  ...
  `}function wt(e,t,s,i){if(!s)return W`<span class="row-value muted">
    ...
    </span>`;switch(t){case"number":return function(e,t,s){const{disabled:i,empty:n,staged:r}=gt(e,t),o=$t(t,"min",0),a=$t(t,"max",100),c=$t(t,"step",1),l=t.attributes.unit_of_measurement??"",d=r??t.state,h=n?"":d,p=s=>{const i=s.target.valueAsNumber;Number.isNaN(i)||vt(e,t,"set_value",{value:i},String(i))};return"slider"===$t(t,"mode","box")?W`
      <input
        class="slider"
        type="range"
        aria-label=${s}
  ...
  `}(e,s,i);case"select":return function(e,t,s){const{disabled:i,empty:n,staged:r}=gt(e,t),o=$t(t,"options",[]),a=r??t.state;return W`
    <select
      class="select-input"
      aria-label=${s}
      ?disabled=${i}
  ...
  `}(e,s,i);case"text":return function(e,t,s){const{disabled:i,empty:n,staged:r}=gt(e,t),o=t.attributes.pattern;return W`
    ...
    />
  `}(e,s,i);case"date":return yt(e,s,i,"date");case"time":return yt(e,s,i,"time");case"datetime":return yt(e,s,i,"datetime");default:return W`<span class="row-value muted">
