...
`;function ut(e){return e&&Ve("ha-icon")?I`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:F}function mt(e){const{config:t}=e;if(!t.header.visible)return F;const s=function(e){const t=e.config.header.icon;if(!1===t)return null;if(t)return t;const s=Ze(e.hass,e.subject);return s?.icon?s.icon:"preset_mode"===e.subject.kind?"mdi:state-machine":"mdi:tune-variant"}(e),i=!1===t.header.subtitle?null:t.header.subtitle??function(e){const{hass:t,subject:s}=e,i=Ze(t,s),n=i?.name??dt(t,"no_mode");if("preset_mode"===s.kind){const{presetMode:e}=s;if(e.source_entity){const s=t.states[e.source_entity];return`${n} · ${dt(t,"follows",{entity:s?.attributes.friendly_name??e.source_entity})}`}return n}return s.presetMode?!1===tt(t,s)?`${n} · ${dt(t,"by_hand")}`:n:`${n} · ${dt(t,"no_preset_mode")}`}(e),n=t.header.icon_color??function(e){const t=Ze(e.hass,e.subject);return t?e.config.modes.colors[t.key]:void 0}(e),{tappable:r}=e;return I`
  ...
  `}(e,t,s,i)}function bt(e){const t=function(e){const{hass:t,subject:s,config:i}=e;if(!i.modes.automatic)return F;const n=Qe(s);if(!n)return F;const r=tt(t,s),o=dt(t,"follows_preset_mode");return I`
    <label class="toolbar">
