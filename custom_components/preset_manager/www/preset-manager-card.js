...
`;function ut(e){return e&&Ve("ha-icon")?I`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:F}function mt(e){const{config:t}=e;if(!t.header.visible)return F;const s=function(e){const t=e.config.header.icon;if(!1===t)return null;if(t)return t;const s=Ze(e.hass,e.subject);return s?.icon?s.icon:"preset_mode"===e.subject.kind?"mdi:state-machine":"mdi:tune-variant"}(e),i=!1===t.header.subtitle?null:t.header.subtitle??function(e){const{hass:t,subject:s}=e,i=Ze(t,s),n=i?.name??dt(t,"no_mode");if("preset_mode"===s.kind){const{presetMode:e}=s;if(e.source_entity){const s=t.states[e.source_entity];return`${n} · ${dt(t,"follows",{entity:s?.attributes.friendly_name??e.source_entity})}`}return n}return s.presetMode?!1===tt(t,s)?`${n} · ${dt(t,"by_hand")}`:n:`${n} · ${dt(t,"no_preset_mode")}`}(e),n=t.header.icon_color??function(e){const t=Ze(e.hass,e.subject);return t?e.config.modes.colors[t.key]:void 0}(e),{tappable:r}=e;return I`
    <div
      ...
      @keydown=${t=>{!r||"Enter"!==t.key&&" "!==t.key||(t.preventDefault(),e.onHeaderClick())}}
    >
      ${s?I`<div class="icon">${ut(s)}</div>`:F}
      <div class="titles">
  ...
  `}function ft(e,t){const s=et(e.subject);s&&e.call(e.hass.callService("preset_manager","set_active_mode",{mode:t},{entity_id:s}))}function _t(e){const t=Ye(e.subject);if(!t.length)return I`<div class="note">${dt(e.hass,"no_modes")}</div>`;const s=Ge(e.hass,e.subject),i=null!==st(e.hass,e.subject);return"dropdown"===e.config.modes.style?function(e,t,s,i){return I`
    <select
      class="select-input"
    ...
    >
      ${null===s?I`<option value="" selected>${dt(e.hass,"no_mode")}</option>`:F}
      ${t.map(e=>I`
  ...
  `}(e,t,s,i):function(e,t,s,i){const{config:n}=e;return I`
    <div class="chips" role="group">
      ${t.map(t=>{const r=t.key===s,o=n.modes.colors[t.key];return I`
          <button
            class="chip"
            type="button"
  ...
  `}(e,t,s,i)}function bt(e){const t=function(e){const{hass:t,subject:s,config:i}=e;if(!i.modes.automatic)return F;const n=Qe(s);if(!n)return F;const r=tt(t,s),o=dt(t,"automatic");return I`
    <label class="toolbar">
      <span class="toolbar-label">${o}</span>
      <span class="switch">
        <input
          type="checkbox"
          role="switch"
  ...
  `}(e),s=function(e){const t=e.config.modes.visible;return"manual"===t?null===st(e.hass,e.subject):"never"!==t}(e)?_t(e):F;return t===F&&s===F?F:I`<div class="section rows">${t}${s}</div>`}function vt(e,t,s,i,n){if(e.stage)return void e.stage(t.entity_id,{service:s,data:i,display:n});const r=t.entity_id.split(".",1)[0];e.call(e.hass.callService(r,s,i,{entity_id:t.entity_id}))}function gt(e,t){const s=e.draft?.get(t.entity_id)?.display;return void 0!==s?{disabled:!1,empty:!1,staged:s}:{disabled:(i=t.state,void 0===i||i===He),empty:De(t.state),staged:void 0};var i}function $t(e,t,s){const i=e.attributes[t];return null==i?s:i}function yt(e,t,s,i){const{disabled:n,empty:r,staged:o}=gt(e,t);let a=o??(r?"":t.state);return void 0===o&&("datetime"===i&&(a=r?"":function(e){const t=new Date(e);if(Number.isNaN(t.getTime()))return"";const s=e=>String(e).padStart(2,"0");return`${t.getFullYear()}-${s(t.getMonth()+1)}-${s(t.getDate())}T${s(t.getHours())}:${s(t.getMinutes())}`}(t.state)),"time"===i&&(a=a.slice(0,5))),I`
    <input
      class="date-input"
    ...
    />
  `}function wt(e,t,s,i){if(!s)return I`<span class="row-value muted">
      ${dt(e.hass,"unavailable")}
    </span>`;switch(t){case"number":return function(e,t,s){const{disabled:i,empty:n,staged:r}=gt(e,t),o=$t(t,"min",0),a=$t(t,"max",100),c=$t(t,"step",1),l=t.attributes.unit_of_measurement??"",d=r??t.state,h=n?"":d,p=s=>{const i=s.target.valueAsNumber;Number.isNaN(i)||vt(e,t,"set_value",{value:i},String(i))};return"slider"===$t(t,"mode","box")?I`
      <input
        class="slider"
        type="range"
        aria-label=${s}
  ...
  `}(e,s,i);case"boolean":return function(e,t,s){const{disabled:i,empty:n,staged:r}=gt(e,t);return I`
    <label class="switch">
      <input
        type="checkbox"
        role="switch"
  ...
  `}(e,s,i);case"select":return function(e,t,s){const{disabled:i,empty:n,staged:r}=gt(e,t),o=$t(t,"options",[]),a=r??t.state;return I`
    <select
      class="select-input"
      aria-label=${s}
      ?disabled=${i}
  ...
  `}(e,s,i);case"text":return function(e,t,s){const{disabled:i,empty:n,staged:r}=gt(e,t),o=t.attributes.pattern;return I`
    <input
      class="text-input"
      type=${"password"===$t(t,"mode","text")?"password":"text"}
      aria-label=${s}
      minlength=${$t(t,"min",0)}
      maxlength=${$t(t,"max",255)}
      pattern=${o??F}
      .value=${n?"":r??t.state}
      ?disabled=${i}
      @change=${s=>{const i=s.target.value;vt(e,t,"set_value",{value:i},i)}}
    />
  `}(e,s,i);case"date":return yt(e,s,i,"date");case"time":return yt(e,s,i,"time");case"datetime":return yt(e,s,i,"datetime");default:return I`<span class="row-value muted">
        ${dt(e.hass,"not_editable")}
      </span>`}}function xt(e,t){return"number"===e&&void 0!==t&&"slider"===$t(t,"mode","box")}function At(e,t){const s=e.config.values.parameters,i=new Map((s??[]).map(e=>[e.parameter,e]));return function(e,t){if(!t)return e.parameters;const s=new Map(e.parameters.map(e=>[e.key,e]));return t.map(e=>s.get(e)).filter(e=>void 0!==e)}(t,s?s.map(e=>e.parameter):null).map(e=>{const t=i.get(e.key);return{parameter:e,label:t?.name??e.name,icon:t?.icon}})}function kt(e,t,s){if(!s)return F;const i=!1===t.icon?void 0:t.icon??(e.config.values.icons?Le(e.hass,t.parameter.entity)?.attributes.icon:void 0);return I`<span class="row-icon">${ut(i)}</span>`}function Et(e,t,s){const{text:i,muted:n}=function(e,t){const s=Le(e.hass,t.entity);return s?s.state===Re?{text:dt(e.hass,"not_set"),muted:!0}:De(s.state)?{text:dt(e.hass,"unavailable"),muted:!0}:{text:We(e.hass,s),muted:!1}:{text:dt(e.hass,"unavailable"),muted:!0}}(e,t.parameter);return I`
    <div class="row">
      <div class="row-label">
  ...
  `}function St(e,t,s,i,n){const r=s?t.parameter.editors[s]:void 0,o=Le(e.hass,r),a=xt(t.parameter.type,o);return I`
  ...
  `}function Mt(e){const t=dt(e.hass,"editing");return I`
    <label class="toolbar">
  ...
  `}function Ct(e){if("preset"!==e.subject.kind)return F;if(!e.config.values.visible)return F;const t=e.subject.preset,s=At(e,t);if(!s.length)return I`<div class="section note">
      ${dt(e.hass,"no_parameters")}
    </div>`;const i=null===e.subject.presetMode?I`<div class="note warning">${dt(e.hass,"orphaned")}</div>`:F,n=function(e,t){return e.config.values.icons||t.some(e=>"string"==typeof e.icon)}(e,s),{editor:r}=e.config,o=()=>I`
    <div class="section rows">
      ${i}${r.confirm?Mt(e):F}
      ${s.map(t=>Et(e,t,n))}
    </div>
  `;if(!r.enabled)return o();if(r.confirm&&!e.editing)return o();if("all"===r.mode){const t=Ye(e.subject);return I`
      <div class="section rows">
        ${i}${r.confirm?Mt(e):F}
        ${s.map(s=>I`
    ...
    `}const a="active"===r.mode?Ge(e.hass,e.subject):e.editMode;return I`
    <div class="section rows">
      ${i}${r.confirm?Mt(e):F}
      ${"picker"===r.mode?function(e){const t=Ye(e.subject);if(t.length<2)return F;const s=Ge(e.hass,e.subject),i=t.find(e=>e.key===s),n=dt(e.hass,e.config.editor.confirm?"mode":"editing"),r="dropdown"===e.config.editor.style?I`
          <select
            class="select-input"
            aria-label=${n}
  ...
  `}(e,t,i):function(e,t){const s=Le(e.hass,t.entity);let i,n=!0;return s?s.state===Re?i=dt(e.hass,"not_set"):De(s.state)?i=dt(e.hass,"unavailable"):(i=We(e.hass,s),n=!1):i=dt(e.hass,"unavailable"),I`<div class="row-value ${n?"muted":""}">${i}</div>`}(e,i)}
    ...
    <div class="section rows">
      ${t.map(t=>{const i=t.entities.active_mode,n=I`
        ...
        `;if(!s){const t=Le(e.hass,i);return I`
            <div class="row">
              ${n}
  ...
  `}(i)} ${function(e){if(!e.config.footer.visible)return F;const t=e.config.footer.content.map(t=>Ot(e,t)).filter(e=>null!==e);return t.length?I`
