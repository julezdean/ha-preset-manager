...
`;function mt(e){return e&&Je("ha-icon")?W`<ha-icon .icon=${e} aria-hidden="true"></ha-icon>`:F}function ft(e){const{config:t}=e;if(!t.header.visible)return F;const s=function(e){const t=e.config.header.icon;if(!1===t)return null;if(t)return t;const s=Qe(e.hass,e.subject);return s?.icon?s.icon:"preset_mode"===e.subject.kind?"mdi:state-machine":"mdi:tune-variant"}(e),i=!1===t.header.subtitle?null:t.header.subtitle??function(e){const{hass:t,subject:s}=e,i=Qe(t,s),n=i?.name??ht(t,"no_mode");if("preset_mode"===s.kind){const{presetMode:e}=s;if(e.source_entity){const s=t.states[e.source_entity];return`${n} · ${ht(t,"follows",{entity:s?.attributes.friendly_name??e.source_entity})}`}return n}if(!s.presetMode)return`${n} · ${ht(t,"no_preset_mode")}`;const r=st(t,s);return null===r?n:`${n} · ${ht(t,r?"automatic":"manual")}`}(e),n=t.header.icon_color??function(e){const t=Qe(e.hass,e.subject);return t?e.config.modes.colors[t.key]:void 0}(e),{tappable:r}=e,o=function(e){const{hass:t,subject:s,config:i}=e;if(!i.header.automatic)return F;const n=et(s);if(!n)return F;const r=st(t,s),o=ht(t,"mode_automatic");return W`
  ...
  `}function _t(e,t,s,i){const{config:n}=e;return W`
    <div class="chips" role="group">
      ${t.map(t=>{const r=t.key===s,o=n.modes.colors[t.key];return W`
          <button
            ...
            @click=${()=>function(e,t){const s=tt(e.subject);s&&e.call(e.hass.callService("preset_manager","set_active_mode",{mode:t},{entity_id:s}))}(e,t.key)}
          >
            ${n.modes.icons?mt(t.icon):F}
            <span>${t.name}</span>
          </button>
        `})}
    </div>
  `}function bt(e){const t=Ze(e.subject);if(!t.length)return W`<div class="note">${ht(e.hass,"no_modes")}</div>`;const s=Xe(e.hass,e.subject);return"preset_mode"===e.subject.kind?function(e,t,s){const{config:i}=e;return W`
  ...
  `}(e,t,i):function(e,t){const s=Be(e.hass,t.entity);let i,n=!0;return s?s.state===De?i=ht(e.hass,"not_set"):Le(s.state)?i=ht(e.hass,"unavailable"):(i=We(e.hass,s),n=!1):i=ht(e.hass,"unavailable"),W`<div class="row-value ${n?"muted":""}">${i}</div>`}(e,i)}
    ...
    <div class="section rows">
      ...
      ${s.map(t=>{const s=t.entities.active_mode,n=W`
        ...
        `;if(!i){const t=Be(e.hass,s);return W`
            <div class="row">
              ${n}
              <div class="row-value ${t&&!Le(t.state)?"":"muted"}">
                ${t&&!Le(t.state)?t.state:ht(e.hass,"no_mode")}
              </div>
            </div>
          `}return W`
  ...
  `:F}(i)}
    ...
    `)}_shell(e){return W`<ha-card>${e}</ha-card>`}_skeleton(){return W`
      <div class="section rows" aria-busy="true" aria-label=${ht(this._hass,"loading")}>
        <div class="skeleton" style="width:45%"></div>
        <div class="skeleton" style="width:70%"></div>
      </div>
    `}_alert(e){return Je("ha-alert")?W`<ha-alert alert-type="warning">${e}</ha-alert>`:W`<div class="fallback-alert" role="alert">${e}</div>`}};Ut.styles=pt,t([ue()],Ut.prototype,"_config",void 0),t([ue()],Ut.prototype,"_structure",void 0),t([ue()],Ut.prototype,"_editModeOverride",void 0),t([ue()],Ut.prototype,"_editing",void 0),t([ue()],Ut.prototype,"_draft",void 0),t([ue()],Ut.prototype,"_error",void 0),Ut=t([le(e)],Ut);const zt={entity:"Entity",header:"Header",modes:"Modes",values:"Values",editor:"Editing",presets:"Presets",footer:"Footer",actions:"Actions",visible:"Show",show:"Show",automatic:"Automatic switch",title:"Title",subtitle:"Subtitle",icon:"Icon",icon_color:"Icon colour",style:"Style",icons:"Show icons",parameters:"Parameters",parameters_note:"Parameters",enabled:"Editable",confirm:"Confirm with a button",mode:"Which mode",default_mode:"Start on",content:"Content",tap_action:"Tap",hold_action:"Hold",double_tap_action:"Double tap"},Ht=["more-info","navigate","url","perform-action","none"];function Rt(e){return{selector:{select:{mode:"dropdown",options:e.map(([e,t])=>({value:e,label:t}))}}}}let Dt=class extends ae{constructor(){super(...arguments),this._config={},this._label=e=>e.name&&zt[e.name]||e.title||e.name||""}set hass(e){this._hass=e,this._structure??=ze(e),this._load(),this.requestUpdate()}get hass(){return this._hass}setConfig(e){this._config=e}disconnectedCallback(){super.disconnectedCallback(),this._unsubscribe?.(),this._unsubscribe=void 0}async _load(){this._hass&&!this._unsubscribe&&(this._unsubscribe=He(this._hass,e=>{this._structure=e}),this._structure=await Ue(this._hass))}_entityPicker(){const e=[...(this._structure?.preset_modes??[]).map(e=>e.entities.mode),...(this._structure?.presets??[]).map(e=>e.entities.active_mode)].filter(e=>Boolean(e));return e.length?{include_entities:e}:{integration:"preset_manager"}}_hasParameterOverrides(){const e=this._config.values,t=e?.parameters;return Array.isArray(t)&&t.some(e=>"string"!=typeof e)}get _subject(){const e=this._config.entity;return this._structure&&"string"==typeof e&&e?ct(this._structure,e):null}_schema(e){const t="preset"===e?.kind,s="preset_mode"===e?.kind,i=e?"preset_mode"===e.kind?e.presetMode.modes:e.preset.modes:[],n=[{name:"entity",required:!0,selector:{entity:this._entityPicker()}},{type:"expandable",name:"header",title:zt.header,schema:[{name:"visible",selector:{boolean:{}}},...s?[]:[{name:"automatic",selector:{boolean:{}}}],{name:"title",selector:{text:{}}},{name:"subtitle",selector:{text:{}}},{name:"icon",selector:{icon:{}}},{name:"icon_color",selector:{ui_color:{}}}]},{type:"expandable",name:"modes",title:zt.modes,schema:[s?{name:"visible",selector:{boolean:{}}}:{name:"visible",...Rt([["always","Always"],["never","Never"],["manual","While the mode can be set by hand"]])},...i.some(e=>e.icon)?[{name:"icons",selector:{boolean:{}}}]:[]]}];if(t){const t=e.preset.parameters.map(e=>({value:e.key,label:e.name}));n.push({type:"expandable",name:"values",title:zt.values,schema:[{name:"visible",selector:{boolean:{}}},this._hasParameterOverrides()?{name:"parameters_note",type:"constant",value:"Renamed parameters are edited in YAML."}:{name:"parameters",selector:{select:{multiple:!0,mode:"list",options:t}}},{name:"icons",selector:{boolean:{}}}]},{type:"expandable",name:"editor",title:zt.editor,schema:[{name:"enabled",selector:{boolean:{}}},{name:"confirm",selector:{boolean:{}}},{name:"mode",...Rt([["picker","Pick a mode in the card"],["active","The active mode"],["all","Every mode"]])},{name:"style",...Rt([["chips","Chips"],["dropdown","Dropdown"]])},{name:"default_mode",...Rt(i.map(e=>[e.key,e.name]))}]})}return s&&n.push({type:"expandable",name:"presets",title:zt.presets,schema:[{name:"show",...Rt([["none","Nothing"],["names","Their names and modes"],["values","Their values as well"],["editable","Their values, editable"]])}]}),n.push({type:"expandable",name:"footer",title:zt.footer,schema:[{name:"visible",selector:{boolean:{}}},{name:"content",selector:{select:{multiple:!0,mode:"list",options:[{value:"preset_mode",label:"Preset mode"},{value:"blueprint",label:"Blueprint"},{value:"source",label:"Source entity"},{value:"last_changed",label:"Last change"}]}}}]},{type:"expandable",title:zt.actions,schema:[{name:"tap_action",selector:{ui_action:{actions:Ht}}},{name:"hold_action",selector:{ui_action:{actions:Ht}}},{name:"double_tap_action",selector:{ui_action:{actions:Ht}}}]}),n}_formData(){try{const t=Me({type:`custom:${e}`,...this._config,entity:this._config.entity??"sensor.placeholder"}),s=t.modes;return{...t,entity:this._config.entity??"",modes:{...s,..."preset_mode"===this._subject?.kind?{visible:"never"!==s.visible}:{}},values:{...t.values,parameters:this._config.values?.parameters}}}catch(e){return{...this._config}}}_valueChanged(e){e.stopPropagation();const t={...e.detail.value},s=t.modes;"preset_mode"===this._subject?.kind&&"boolean"==typeof s?.visible&&(t.modes={...s,visible:s.visible?"always":"never"}),Ie(this,"config-changed",{config:Te(t)})}render(){if(!this._hass)return F;const e=this._subject;return W`
      <ha-form
        .hass=${this._hass}
        .data=${this._formData()}
      ...
      ${this._config.entity&&!e&&this._structure?W`<p style="color: var(--error-color)">
            ${ht(this._hass,"not_found",{entity:String(this._config.entity)})}
          </p>`:F}
    `}};t([ue()],Dt.prototype,"_config",void 0),t([ue()],Dt.prototype,"_structure",void 0),Dt=t([le(`${e}-editor`)],Dt),window.customCards=window.customCards??[],window.customCards.some(t=>t.type===e)||window.customCards.push({type:e,name:"Preset Manager",description:"The values of a preset, or the modes of a preset mode, with the mode it is on right now.",preview:!1,documentationURL:"https://github.com/julezdean/ha-preset-manager"}),console.info("%c PRESET-MANAGER-CARD %c 0.4.0-beta.6 ","color: white; background: #03a9f4; font-weight: 700;","color: #03a9f4; background: white; font-weight: 700;");
