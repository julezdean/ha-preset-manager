"""Time platform: editors for time-of-day mode values."""
...
async def async_setup_entry(
    ...
    for subentry_id, coordinator in runtime.presets.items():
        entities = [
            ModeParameterTime(coordinator, mode, parameter)
            for parameter in coordinator.config.parameters
            if get_parameter_type(parameter.type).editor_platform is Platform.TIME
...
class ModeParameterTime(ModeValueEditorEntity, TimeEntity):
    """Editable time of one parameter in one mode."""

    @property
    def native_value(self) -> time | None:
        """Return the configured value."""
        value = self.raw_value
        return None if value is None else dt_util.parse_time(str(value))
